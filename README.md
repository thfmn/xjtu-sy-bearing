...
# Step 5a: Train deep learning models (requires GPU)
...
# Step 5b: Generate sonified audio from vibration signals (optional)
#   → outputs/audio/  (.wav files for dashboard Audio tab)
python scripts/05_create_audio.py

# Step 6a: Evaluate and aggregate DL model results
#   → outputs/evaluation/dl_model_summary.csv
python scripts/06_evaluate_dl_models.py

# Step 6b: Generate onset detection labels with statistical detectors
#   → outputs/onset/onset_labels_auto.csv  (15 rows, one per bearing)
python scripts/06_generate_onset_labels.py

# Step 7: Train LSTM onset classifier (requires GPU)
